...
| No double-claim (nullifiers or equivalent) | ✅ marker PDA seeded by `compute_claim_marker(distribution_id, nullifier)`; `init` fails on the second claim. Tested three ways: `airdrop-core` (circuit level), `claim-verifier-tests` (the deployed binary through the sequencer's executor, on every push), and **live on chain** — `scripts/prove-one-claim.sh` step 6 resubmits a claim it has just landed, under a *different* signer, and fails the run if that produces a transaction instead of `AccountAlreadyInitialized`. |
